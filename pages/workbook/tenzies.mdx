...
Now we are getting values from our state when the component mounts and we need the die component to display this value. So, we want to pass it down as props. Since we are using typescript, we would need to specify the type for the props that will be passed.
