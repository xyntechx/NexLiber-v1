    ...
    <div>
      ...
      <main className={styles.main}>
        ...
        <section className={styles.content}>
          ...
          />
          <p className={styles.paragraph}>
            Alright, let&apos;s install one more dependency in the command
            prompt:
          </p>
          ...
          <p className={styles.paragraph}>
            This workbook will guide you through the process of interacting with
            the blockchain, while introducing you to some of more important
            technicalities. At the end, you will have a command line interface
            to query balances, use smart contracts and interact with ERC-20
            tokens.
          </p>
          <h1 className={styles.header}>Blockchain: in brief</h1>
          <p className={styles.paragraph}>
            In grossly simplified terms, the Ethereum blockchain, like many
            others, is simply a centralised ledger (basically a big dictionary
            of addresses and balances), which is mutated through transactions,
            and controlled through the consensus mechanism. Consider reading{" "}
            <Link href="https://www.preethikasireddy.com/post/how-does-ethereum-work-anyway">
              <a className={styles.link} target="_blank">
                this
              </a>
            </Link>{" "}
            comprehensive introduction to understand blockchain better.
          </p>
          <h1 className={styles.header}>Querying Balances</h1>
          <p className={styles.paragraph}>
            Every blockchain has a native token, which can be used to pay
            transaction fees (gas), run smart contracts, or purchase real-world
            goods.
            <br />⇒ for example, ether on Ethereum, Sol on Solana, etc.
          </p>
          <p className={styles.paragraph}>
            To interact with this native token on the Ethereum blockchain,
            let&apos;s first clear our{" "}
            <span className={styles.highlight}>index.js</span>, and import the
            ethers library.
          </p>
          <Code code='const { ethers } = require("ethers");' />
          <p className={styles.paragraph}>
            To connect with the blockchain, a{" "}
            <span className={styles.highlight}>Provider</span> must first be
            instantiated.
          </p>
          <Code code="const provider = ethers.getDefaultProvider();" />
          <p className={styles.paragraph}>
            With this, we now gain access to a range of asynchronous utility
            functions. To provide a testbed for testing, let&apos;s create a
            quick async/await function:
          </p>
          <Code
            code={`...
...
getBalance();`}
          />
          <p className={styles.paragraph}>
            Let&apos;s first query the balance of a random wallet, and put this
            following code <u>inside the function</u> you just created:
          </p>
          <Code
  ...
  console.log(balance);`}
          ...
          <p className={styles.paragraph}>
            If you&apos;ve gotten a response from the last step, you might
            notice the weird <span className={styles.highlight}>BigNumber</span>{" "}
            data type. This data type contains a hexstring (under{" "}
            <span className={styles.highlight}>_hex</span>), which must be
            parsed to obtain a floating point value.
          </p>
          <p className={styles.paragraph}>
            To do this, let&apos;s first import{" "}
            <span className={styles.highlight}>formatEther</span> from the{" "}
            <span className={styles.highlight}>ethers/utils</span> library
            first.
          </p>
...
`}
          />
          <p className={styles.paragraph}>
            …and then run the server with the `node ./index.js` command from
            earlier.
          </p>
          <Quiz
          ...
          <p className={styles.paragraph}>
            Alright, final step: let&apos;s get users to key in addresses they
            want to query, instead of hardcoding it in. First, instantiate a
            variable to hold this address, adding{" "}
          ...
          <p className={styles.paragraph}>
            Then, import <span className={styles.highlight}>isAddress</span> and
            this new library:
          </p>
          <Code
            code={`…
...
`}
          ...
          <p className={styles.paragraph}>
            ...
            <span className={styles.highlight}>const address = …</span>{" "}
            statement with
          </p>
          <Code
            code={`  while (true) {
...
`}
          ...
          <p className={styles.paragraph}>
            ...
            contracts, which can be used to create a separate token on the same
            blockchain. In this workbook, we will interact with Dai, a ERC-20
            (i.e. non-native) token. Let&apos;s first create a new asynchronous
            function:
          </p>
          <Code
            code={`…
...
`}
          />
          <p className={styles.paragraph}>
            In the function, we first create an abstraction of the smart
            contract:
          </p>
          <Code
...
`}
          ...
          <p className={styles.paragraph}>
            ...
            <span className={styles.highlight}>daiAbi</span> is needed to
            specify which functions from the contract will be retrieved, and
            defined locally.
          </p>
          <p className={styles.paragraph}>
...
`}
          ...
          />
          <p className={styles.paragraph}>
            Again, it is worth noting that values are stored as integers on the
            blockchain (and in balance._hex). This balance is actually raised to
            10<sup>18</sup> of the true value (for dai and ether), providing 18
            artificial decimal places.
          </p>
          <p className={styles.paragraph}>Now, we log out the balance:</p>
          <Code
            code={`  console.log(
...
`}
          ...
          <p className={styles.paragraph}>
            If you would like to see the complete code, you can do so{" "}
          ...
          <p className={styles.paragraph}>
            If you have questions about anything related to NexLiber or this
            workbook, you&apos;re welcome to ask our community on{" "}
